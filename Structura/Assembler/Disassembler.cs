...
namespace Structura.Assembler
{
    public static class Disassembler
    {
        ...
        public static List<string> Disassemble(Int64[] machineCodeAsInt64Array, bool withIC)
        {
            byte[] machineCodeAsByteArray=new byte[machineCodeAsInt64Array.Length*8];

            //Kopiere Int64 Array in ByteArray
            for(int i=0;i<machineCodeAsInt64Array.Length;i++)
            {
                byte[] i64=BitConverter.GetBytes(machineCodeAsInt64Array[i]);
                Array.Copy(i64, 0, machineCodeAsByteArray, i*8, 8);
            }

            return Disassemble(machineCodeAsByteArray, withIC);
        }
